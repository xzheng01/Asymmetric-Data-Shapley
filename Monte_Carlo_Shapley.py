...
class Monte_Carlo_Shapley(object):

    def __init__(self, X, y, X_test, y_test, directory, task='classification',
           model_family='Tree', metric='accuracy', continued=True, max_permutations=10000,
           seed=None, **kwargs):
    ...
    def create_results_placeholder(self):
        tmc_dir = os.path.join(self.directory, 'tmc_shapley_model={}_metric={}.pkl'.format(self.model_family, self.metric))
    ...
    def run(self, save_every, err, tolerance=0, loo_run=True):
        ...
        Args:
            ...
            tmc_run: If True, computes TMC-symmetric-Shapley values.
        ...
        while tmc_run:
            ...
            print('self.num_permutations', self.num_permutations, 'self.max_permutations', self.max_permutations)
            if error(self.marginals_tmc) < err or self.num_permutations >= self.max_permutations:
                tmc_run = False
            else:
                self.tmc_shap(save_every, tolerance=tolerance) # save marginal contributions every n iterations
                self.vals_tmc = np.mean(self.marginals_tmc, 0)
                self.save_shapley_results()

    def save_loo_results(self):
        """Saves results computed so far."""
        loo_dir = os.path.join(self.directory, 'loo_model={}_metric={}.pkl'.format(self.model_family, self.metric))
        pkl.dump({'loo': self.vals_loo}, open(loo_dir, 'wb'))

    def save_shapley_results(self):
        tmc_dir = os.path.join(self.directory, 'tmc_shapley_model={}_metric={}.pkl'.format(self.model_family, self.metric))
        pkl.dump({'vals_tmc':self.vals_tmc, 'marginals_tmc': self.marginals_tmc,
                  'idxs_tmc': self.idxs_tmc}, open(tmc_dir, 'wb'))

    def tmc_shap(self, iterations, tolerance):
