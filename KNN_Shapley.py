...
def Logistic_LOO(X_ori_aug, y_ori_aug, X_test, y_test, imbalance):
    N = X_ori_aug.shape[0]
    loos = np.zeros(N)
    for i in range(N):
        Log_reg1 = LogisticRegression(max_iter=5000, random_state=666)
        Log_reg1.fit(X_ori_aug, y_ori_aug)
        preds1 = Log_reg1.predict(X_test)
        if imbalance:
            acc1 = roc_auc_score(y_test, preds1)
        else:
            acc1 = accuracy_score(y_test, preds1)

        Log_reg2 = LogisticRegression(max_iter=5000, random_state=666)
        X_ori_aug_i = np.delete(X_ori_aug, i, axis=0)
        y_ori_aug_i = np.delete(y_ori_aug, i, axis=0)
        Log_reg2.fit(X_ori_aug_i, y_ori_aug_i)
...
def KNN_LOO(X_ori_aug, y_ori_aug, X_test, y_test, K, imbalance):
    ...
    loos = np.zeros(N)
    for i in range(N):
        KNN1 = KNN(n_neighbors=K, weights='uniform')
        KNN1.fit(X_ori_aug, y_ori_aug)
        ...
        KNN2 = KNN(n_neighbors=K, weights='uniform')
        X_ori_aug_i = np.delete(X_ori_aug, i, axis=0)
        y_ori_aug_i = np.delete(y_ori_aug, i, axis=0)
        KNN2.fit(X_ori_aug_i, y_ori_aug_i)
